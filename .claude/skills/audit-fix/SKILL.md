...
Spawn `engineer` agent via Agent tool. Pass ALL approved findings one bundle:
...
Wait for engineer return. Read changed files, confirm fixes applied.
...
Any step fails: **STOP**. Report failure. No commit. User resolves, re-invokes.

## Step 5: Commit plan (user approval gate)

Run `git diff --stat HEAD` and `git diff HEAD`. Nothing changed → report "no changes to commit", stop.
...
**Wait for explicit user approval** before committing. User may adjust grouping, edit messages, drop batches.
